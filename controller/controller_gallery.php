...
class Controller_gallery extends Controller
{
    public function __construct($contant, $templateView, $routes = null) {
        ...
        if (isset($_POST['text']) && !empty($_POST['text'])) {
            ...
            $q = array(htmlentities($_POST['text'], ENT_SUBSTITUTE|ENT_QUOTES|ENT_HTML5), $_SESSION['user']['usr_name']);
            $q = json_encode($q);
            echo $q;
            exit ;
        }
        ...
        $photoTableData = $this->model->getPhotoTable($_SESSION['user']['user_id']);
